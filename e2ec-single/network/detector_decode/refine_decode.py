...
class Decode(torch.nn.Module):
    ...
    def test_decode(self, cnn_feature, output, K=100, min_ct_score=0.05, ignore_gloabal_deform=False):
        ...
        poly_init, detection, score ,class_num= decode_ct_hm(torch.sigmoid(hm_pred), wh_pred,
    ...
    #可视化结果
    def visualize_decode(self, cnn_feature, output, K=100, min_ct_score=0.05, ignore_gloabal_deform=False):
        hm_pred, wh_pred = output['ct_hm'], output['wh']
        poly_init, detection, score ,class_num= decode_ct_hm(torch.sigmoid(hm_pred), wh_pred,
                                            K=K, stride=self.stride)
        valid = detection[0, :, 2] >= min_ct_score
    ...
    def forward(self, data_input, cnn_feature, output=None, is_training=True, is_visualize=False, ignore_gloabal_deform=False):
        if is_training:
            self.train_decode(data_input, output, cnn_feature)
        elif is_visualize:
            score ,class_num=self.visualize_decode(cnn_feature, output, min_ct_score=self.min_ct_score,
                                                ignore_gloabal_deform=ignore_gloabal_deform)
            return score ,class_num
        else:
             #score ,class_num=self.test_decode(cnn_feature, output, min_ct_score=self.min_ct_score,
                             #ignore_gloabal_deform=ignore_gloabal_deform)
