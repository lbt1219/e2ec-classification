...
class Network(nn.Module):
    ...
    def forward(self, x, batch=None):
        ...
        #如果有就测试
        elif 'test' in batch['meta'] and 'vis' not in batch['meta']:
            with torch.no_grad():
                if self.test_stage == 'init':
                    ignore = True
                else:
                    ignore = False
                #score ,class_num = self.train_decoder(batch, cnn_feature, output, is_training=False, ignore_gloabal_deform=ignore)
                self.train_decoder(batch, cnn_feature, output, is_training=False, ignore_gloabal_deform=ignore)
        elif 'test' in batch['meta'] and 'vis' in batch['meta']:
            with torch.no_grad():
                if self.test_stage == 'init':
                    ignore = True
                else:
                    ignore = False
                score ,class_num = self.train_decoder(batch, cnn_feature, output, is_training=False, is_visualize=True, ignore_gloabal_deform=ignore)
